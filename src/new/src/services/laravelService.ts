...
const generatedAppKey = () => {
  const key = execSync('echo "base64:$(openssl rand -base64 32)"', { encoding : 'utf8' })
  return key.trim()
}
...
export function LaravelService(_context: any): ServiceDescriptor {
  ...
  const dbServiceName = `${_context.name}_laravel_${_context.db ? _context.db : 'postgres'}`
  ...
  const dbNetwork = `${_context.db ? _context.db : 'postgres'}_${dbServiceName}_network`
  ...
  return {
    ...
    templates: apply(templates, [template({ dbNetwork, dbHost, generatedAppKey: generatedAppKey(), ..._context, ...strings })]),
  };
}
